<template>
  <div style="display: flex;flex-direction: column;height: 100%">
    ...
    <el-table
      ...
      <el-table-column fixed prop="userno" label="学号"></el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="addEditVisable">
      <el-form :model="form">
        ...
        <el-form-item label="学号" v-if="title=='修改学生信息'? false : true" :label-width="formLabelWidth">
          <el-input v-model="form.userno" :value="form.userno" autocomplete="off" ></el-input>
        </el-form-item>
        <el-form-item label="密码"  :label-width="formLabelWidth" v-if="title=='新增学生'? false : true">
        ...
        <el-form-item label="班级" :label-width="formLabelWidth">
          <el-select :data="classList"  v-model="form.classno" placeholder="请选择班级">
            <el-option v-for="(item,index) in classList" :key="index" :label="item.classname" :value="item.classno"></el-option>
          </el-select>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    //获取人数未满的班级
    getClassList() {
      this.axios.get(`/exam/classes/selectNotFull`).then((res) => {
      ...
      if (this.title=="新增学生"){
        if (this.form.userno != ''){
          this.axios.post(`/exam/user/insert`,this.form)
            .then(res=>{
              if(res != 0){
                this.$refs.upload.submit() //手动提交图片
